...
class server
{
    ...
    server(string p_det_user, string p_det_pasd)
    {
        username=p_det_user;
        password=p_det_pasd;
        frnd_datal.clear();
...
class ser
{
    ...
    ser(int conf_det1, server conf_det2)
    {
        sockfd=conf_det1;
        user=conf_det2;
    }
    ser(int conf_det1, server conf_det2, string ipaddr, int port){
