...
#include "client.h"
        ...
        *local_detailofserver = dif_server_sock;
        ...
        cout<<"Password:";
        cin>>local_pswdr;
        local_getmessgstr=register_msg(userName,local_pswdr);
        ...
        if(!stat_oflogin){
        cout<<"Presently no user is as logged in"<<endl;
        continue;
        }
        ...
        getline(local_storagebufferformgs,local_sendingusr,' ');
        getline(local_storagebufferformgs,local_genterated_livestr,'~');
        local_getmessgstr=invite_msg(local_sendingusr,local_genterated_livestr);
        write(dif_server_sock, local_getmessgstr.c_str(), strlen(local_getmessgstr.c_str())+1);
        }
        ...
        local_getmessgstr=invite_acpt(local_sendingusr,local_genterated_livestr);
        write(dif_server_sock, local_getmessgstr.c_str(), strlen(local_getmessgstr.c_str())+1);
        }
        }
        }
        ...
        client class_var_userdetailfrnd=iter->second;
        ...
        string loc_payload=send_msg(userName,p_user_to_user_frnd_msg);
        ...
        *loc_sockedhpoint = loc_deschandler;
        ...
        string login_msg(string p_datadet, string p_datapd)
        {
        ...
        if(iter!=list_vect_onlinefrnd.end()){
        client onlineFriend=iter->second;
        ...
        string register_msg(string p_datadet, string p_datapd)
        {
        ostringstream ltr_ptre;
        string ltr_datagen2;
        ...
        string invite_acpt(string p_datadet, string p_data1){
        ostringstream ltr_ptre;
        string ltr_datagen2;
        ...
        string invite_msg(string p_datadet, string p_data1)
        {
        ostringstream ltr_ptre;
        string ltr_datagen2;
...
//cout<<"debug cliacpt_connection"<<endl;
        ...
        *loc_machinegg = p_py;
        ...
        string seng_msg(string p_datadet, string p_data1)
        {
        ostringstream ltr_ptre;
        string ltr_datagen2;
        ...
        list_vect_onlinefrnd.clear();
        setdatatosock(dif_server_sock,logout_msg());
        stat_oflogin=false;
        if(p_klir){
        client_rcv_first_msg();
        }
